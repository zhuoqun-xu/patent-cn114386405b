...
医療ソフトウェアのテストには、十分な量があり、複数の関連テーブルにまたがる処理も含めて、対象の業務場面に適したデータが必要です。実際の診療記録は機微性が高く、開発者が利用できない場合があります。過去のデータを人手で組み合わせる方法は効率が低く、コストも高くなります。データ量が少ない場合や、項目間の関係を無視して値を生成した場合には、複数テーブルにまたがる処理を十分に検証できません。私はテーブル情報から項目の意味と関連関係を解析し、利用可能なデータ源から生成規則を導き、ソフトウェアのテストに使う医療データを自動生成する方法を設計しました。

...
医疗软件测试需要数量足够、符合业务场景的数据，其中也包括涉及多张关联表的场景。真实临床记录具有敏感性，软件开发者未必能够取得。人工组装历史数据制作测试集效率较低、成本较高；如果数据量不足，或生成字段值时忽略了字段之间的关联，就难以有效测试涉及多张表的功能。我设计的方法读取数据表信息，识别字段含义与关联关系，结合可用数据源形成生成规则，并自动生成用于软件测试的医疗数据。
